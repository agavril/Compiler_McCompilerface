...
class AbstractSyntaxTree{
	...
	public void expr_end(int fooflag) {
		...
		if (ps == null) {
			...
			if ((table.find(root.val) == null) && !root.val.contains("!T") && fooflag != 1) { // check to see if symbol already exists
				if (fooflag != 3 && !root.val.contains("!")) {
					tmp_cnt = tmp_cnt + 1;
			...
			if (table.foo.findVar(ps.id) != null) {
				ac.addLast(new ACNode("STORE"+type, root.val, null, table.foo.findVar(ps.id).value));
			}
			else {
	...
	public void post_traversal (ASTNode n) {
		...
		if (n.val.equals("+")) {
			...
			else if (table.foo.findInput(n.val) != null) {
				n.val = table.foo.findInput(n.val).value;
				acnode = null;
			}
			else if (table.foo.findVar(n.val) != null) {
				n.val = table.foo.findVar(n.val).value;
				acnode = null;
			}
			else if (table.find(n.val) != null) {
